...
# -*- coding: utf-8 -*-
"""
توليد أصول QR Code — حزمة طباعة/إيميل للعميل.

المخرجات (في exports/) لكل رابط في TARGETS:
  {prefix}-qr-print.png   1181x1181px  (~10 سم @ 300 DPI — للطباعة)
  {prefix}-qr-email.png   512x512px    (للإيميل والسوشيال)
  {prefix}-qr.svg         متجه (مع الشعار مضمّن Base64)
  {prefix}-qr-poster.pdf  ورقة A4 — QR + التسميات
"""
import base64
import pathlib
import sys
...
# قائمة الروابط — أضف/عدّل حسب الحاجة
TARGETS = [
    {
        "url": "https://tedxalfalahyouth.com",
        "prefix": "website",
        "title_ar": "زوروا موقعنا",
        "caption_ar": "امسح الرمز لزيارة موقع الفعالية",
        "title_en": "Visit Our Website",
        "label_en": "TEDxAlFalah Youth",
    },
    {
        "url": "https://www.instagram.com/tedxalfalahyouth/",
        "prefix": "instagram",
        "title_ar": "تابعونا",
        "caption_ar": "امسح الرمز لمتابعتنا على إنستغرام",
        "title_en": "Follow Us",
        "label_en": "@tedxalfalahyouth",
    },
]
...
def build_qr(size_px: int, url: str) -> Image.Image:
    qr = qrcode.QRCode(
...
def make_poster(qr: Image.Image, target: dict) -> None:
    ...
    center_text(250, ar(target["title_ar"]), font(FONT_AR_BOLD, 130), RED)
    center_text(470, target["label_en"], font(FONT_EN_BOLD, 62), (20, 20, 20))
    d.rectangle((W // 2 - 260, 620, W // 2 + 260, 624), fill=RED)

    qr_y = 700
    poster.paste(qr.convert("RGB"), ((W - SIZE_PRINT) // 2, qr_y))

    center_text(1980, ar(target["caption_ar"]), font(FONT_AR, 46), (20, 20, 20))
    center_text(2120, target["url"], font(FONT_EN, 38), (110, 110, 110))
    center_text(2290, ar(target["title_en"]), font(FONT_AR_BOLD, 44), RED)

    poster.save(EXPORTS / f"{target['prefix']}-qr-poster.pdf", "PDF", resolution=300.0)
...
def main() -> int:
    ...
    for target in TARGETS:
        ...
        qr_big = build_qr(SIZE_PRINT, target["url"])
        qr_big.save(EXPORTS / f"{prefix}-qr-print.png")
        build_qr(SIZE_EMAIL, target["url"]).save(EXPORTS / f"{prefix}-qr-email.png")

        qr = qrcode.QRCode(error_correction=ERROR_CORRECT_H, box_size=1, border=4)
        qr.add_data(target["url"])
        qr.make(fit=True)
        (EXPORTS / f"{prefix}-qr.svg").write_text(make_svg(qr), encoding="utf-8")

        make_poster(qr_big, target)

    for f in sorted(EXPORTS.glob("*-qr-print.png")):
