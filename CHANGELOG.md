...
## [v1.0.0-credentials-xlsx] — 2026-08-20 · تسليم ملف اعتمادات العميل

### Added
- **`exports/tedxTeam credentails.xlsx`** (بطلب العميل): ملف Excel بأربع صفحات — الروابط / لوحات الدخول وكلمات المرور / مفاتيح البيئة / سجلات DNS — بقيم كاملة من `CREDENTIALS.md` (نسخة كاملة شاملة VPS/Panel وGOOGLE_PRIVATE_KEY). أُنشئ عبر `gen_credentials_xlsx.py` (مرفوض من git لاحتوائه أسرارًا) — الملف نفسه في `exports/` غير متتبع أصلًا. **تنبيه أمني**: الملف بلا تشفير — يُسلَّم عبر قناة آمنة فقط.

---

## [v1.0.0-remove-video-link] — 2026-08-20 · حذف رابط الفيديو من فورم التقديم

### Changed
- **حذف حقل رابط الفيديو نهائيًا** (قرار العميل — المساران young/expert): أُزيل من مخطط zod (العميل `ApplicationForm.tsx` والخادم `apply/route.ts`)، من واجهة الفورم، من رؤوس/صفوف Google Sheets (19 عمودًا الآن)، ومن الإشعار الإداري — الطلبات تُقبل بلا `videoLink`. حُذفت مفاتيح الترجمة الخاصة به وتُعدّل `whatToExpect`. خطوة "Video to be uploaded" في الخط الزمني أُبقيت (حذف الـ input فقط). الوثائق 05/09/11/12 وDOCUMENTATION حُدّثت.
