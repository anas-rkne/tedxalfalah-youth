...
### 5.1.1 ⚠️ مهمة متابعة — تدوير اعتمادات سُرِّبت في ملف `t` (مكتشف 2026-09)

خلال جلسة تطوير، وُجد ملف بجذر المشروع اسمه **`t`** يحوي أسرار إنتاج حقيقية
(كلمة مرور SMTP من Hostinger، `ADMIN_PASSWORD`، مفاتيح Turnstile/Google) في «working tree»
لملف متتبَّع سابقًا في git (كان فارغًا في الـ HEAD الأصلي). الأسرار **لم تُرفع** في أي commit.

الإجراءات المنفَّذة:
- أُضيف `/t` إلى `.gitignore`.
- أُزيل تعقُّب `t` من الفهرس عبر `git rm --cached t` (الملف باقٍ على القرص، وسيُزال من التعقّب عند أول commit).

**يجب تنفيذه (قيد المتابعة):**
1. **دوِّر الاعتمادات التالية فورًا** لأنها ظهرت في جلسة/بيئة شاهدها طرف تلقائي:
   - `SMTP_PASS` (صندوق `marhaba@tedxalfalahyouth.com`) → غيّره في hPanel ثم حدِّث `.env.local`.
   - `ADMIN_PASSWORD` → غيّره عبر لوحة `/admin/applications` (تغيير كلمة المرور) ثم حدِّث `.env.local`.
   - إن أمكن، جدد مفاتيح Turnstile من Cloudflare.
2. بعد التدوير، احذف الملف `t` نهائيًا من القرص (لم يعد له حاجة) — لن يُرتفع لأنه متجاهل.
3. لا تُعد إنشاء ملفات أسرار بجذر المشروع بأسماء غير محمية؛ استخدم دائمًا `.env.local` (المحمي بـ `.env*`).
