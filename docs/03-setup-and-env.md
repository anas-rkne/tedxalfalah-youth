...
| 4 | `NEXT_PUBLIC_VENUE_MAP_URL` | خريطة Google المضمّنة بصفحة المكان (frame) | `src/app/[locale]/venue/page.tsx:61` | `https://www.google.com/maps?q=Dubai&output=embed` | رابط مشاركة Google Maps (يُسمح له في CSP: `frame-src ...google.com/maps` — `next.config.ts:30`) |
| 5 | `ALLOWED_API_ORIGINS` | النطاقات المسموح لها باستدعاء الـ API (مفصولة بفاصلة) — تتجاوز القائمة الثابتة | `src/lib/cors.ts:9-11` (يُقسم بـ `split(",")`؛ القائمة الأصلية أسطر 3–7) | `https://www.tedxalfalahyouth.com,https://tedxalfalahyouth.com,http://localhost:3000` | دومين الإنتاج + أي نطاق إضافي |
...
| 11 | `ADMIN_APPLICATIONS_EMAIL` | صندوق استلام إشعار كل طلب Apply (بكامل البيانات) | `src/app/api/apply/route.ts:140` (افتراضي `apply@tedxalfalahyouth.com` في الكود) | `apply@tedxalfalahyouth.com` — **مفعّل حاليًا** (اختبار إرسال ناجح) | Hostinger |
| 12 | `CONTACT_EMAIL` | صندوق رسائل فورم التواصل العامة (General/Speaking/Volunteering) | `src/app/api/contact/route.ts:72` (الافتراضي الأخير لـ `inbox`) | `marhaba@tedxalfalahyouth.com` — **مفعّل حاليًا** | Hostinger |
| 13 | `PARTNER_EMAIL` | صندوق الرعايات (رسائل Sponsorship + استفسارات الشراكة) | `src/app/api/contact/route.ts:67` + `src/app/api/partner-inquiry/route.ts:54` | `partners@tedxalfalahyouth.com` — **مفعّل حاليًا** (تحقق من الاسم النهائي للصندوق في hPanel) | Hostinger |
| 14 | `MEDIA_EMAIL` | صندوق استفسارات الإعلام (Media) | `src/app/api/contact/route.ts:68` | `media@tedxalfalahyouth.com` — **مفعّل حاليًا** | Hostinger |
| 15 | `GOOGLE_SHEET_ID` | معرّف Google Sheet لحفظ طلبات Apply | `src/app/api/apply/route.ts:83` (الحارس `hasGoogleSheetConfig` أسطر 200–203) | **(فارغ)** — غير مفعّل حتى الآن | Google Sheets (جلسة العميل المعلقة) |
| 16 | `GOOGLE_SERVICE_ACCOUNT_EMAIL` | بريد Service Account المصرح له بالكتابة في الـ Sheet | `src/app/api/apply/route.ts:77` | **(فارغ)** | Google Cloud (خدمة المسؤول) |
| 17 | `GOOGLE_PRIVATE_KEY` | مفتاح Service Account الخاص — يُعقَّم عبر `sanitizePrivateKey` | `src/app/api/apply/route.ts:78` + المعالج في `src/lib/sanitize.ts:26-32` | **(فارغ)** | Google Cloud — انظر تحذير الأمان (5.4) |
| 18 | `NEXT_PUBLIC_TURNSTILE_SITE_KEY` | مفتاح الموقع العام (يظهر للمتصفح — آمن للكشف) | `src/components/ui/TurnstileWidget.tsx:25` (بدونه لا يُعرض الودجت أصلًا، سطر 35/62) | **(فارغ)** | Cloudflare → Turnstile (سجّل مجانًا) |
| 19 | `TURNSTILE_SECRET_KEY` | المفتاح السري للتحقق من جانب الخادم (سري تمامًا) | `src/lib/turnstile.ts:1` (الحارس)، و`:29` (الطلب لـ siteverify) | **(فارغ)** — بدونه: Fail-open | Cloudflare |
| 20 | `UPSTASH_REDIS_REST_URL` | رابط REST لـ Upstash Redis | `src/lib/rate-limit.ts:5` (الحارس) و`:12` (العميل) | **(فارغ)** — بدونه: Fail-open | Upstash (console.upstash.com) |
| 21 | `UPSTASH_REDIS_REST_TOKEN` | رمز REST لـ Upstash Redis | `src/lib/rate-limit.ts:5` و`:13` | **(فارغ)** | Upstash |
...
| 24 | `SANITY_STUDIO_PROJECT_ID` | معرّف مشروع Sanity Studio (بيئة الستوديو المنفصل) | `studio/sanity.config.ts:19` و`studio/sanity.cli.ts:5` (افتراضي `hisn3dku` في كليهما) | **(فارغ في example)** — الافتراضي يعمل الآن | Sanity.io |
| 25 | `SANITY_STUDIO_DATASET` | مجموعة بيانات الستوديو | `studio/sanity.config.ts:20` و`studio/sanity.cli.ts:6` (افتراضي `"production"`) | **(غير مذكور في example)** — الافتراضي يعمل | Sanity.io |
| 26 | `SANITY_WEBHOOK_SECRET` | مفتاح Webhook للتحديث الفوري عند تعديل المحتوى (revalidate) | `src/app/api/revalidate/route.ts:6` (التحقق أسطر 16–29: إن غير موجود → يُقبل أي Webhook؛ إن موجود → يلزم تطابقه) | **(فارغ)** — إنتاجيًا **مطلوب تعبئته** | أي قيمة عشوائية قوية + نفس القيمة في Sanity webhook |
...
4. فعّل بالترتيب عند توفر الحسابات: **Turnstile** (حماية) → **Upstash** (حد الطلبات) → **Google** (حفظ Apply في Sheet) → **Sanity Webhook** (تحديث فوري).
