...
> ⚠️ **ثغرة موثقة (ناقص من الخريطة):** صفحة **`/speakers` غير موجودة في `ROUTES`** رغم أنها صفحة فعلية (`src/app/[locale]/speakers/page.tsx`). الصفحة مفتوحة دائمًا (تعرض «قريبًا» عند الفراغ — بلا أعلام تحكم). عند الإعلان الرسمي عن المتحدثين يجب إضافة `"/speakers"` إلى `ROUTES` (سطر 6–18) وإعادة البناء. **لاحظ أيضًا:** الخريطة ثابتة — لا تُولَّد صفحات Sanity الديناميكية (متحدث/جلسة) فيها.
