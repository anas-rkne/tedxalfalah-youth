...
### 1.2 `sitemap.ts` — خريطة الموقع (30 سطرًا)

**المسارات المولّدة** (`ROUTES` — أسطر 6–11، 4 مسارات):
`/` · `/team` · `/apply` · `/thank-you` — (بعد إخفاء بقية الصفحات بطلب العميل؛ تُستعاد بإضافتها للقائمة وإعادة البناء)

**آلية التوليد** (أسطر 13–28): `routing.locales.flatMap` — لكل مسار × لكل لغة من `["en", "ar"]` (`src/i18n/routing.ts:4`) تُنشأ نسخة، بمجموع **8 عناوين URL**:
...
```
أي لكل صفحة نسختاها: `en → https://…/en/<route>` و`ar → https://…/ar/<route>` — تُخبر Google أن `/en/apply` و`/ar/apply` نفس المحتوى بلغتين، فتُفهرس النسخة الصحيحة حسب لغة الباحث.

> ⚠️ **ثغرة موثقة (ناقص من الخريطة):** صفحة **`/speakers` غير موجودة في `ROUTES`** رغم أنها صفحة فعلية (`src/app/[locale]/speakers/page.tsx`). الصفحة مفتوحة دائمًا (تعرض «قريبًا» عند الفراغ — بلا أعلام تحكم). عند الإعلان الرسمي عن المتحدثين يجب إضافة `"/speakers"` إلى `ROUTES` (سطر 6–11) وإعادة البناء. **لاحظ أيضًا:** الخريطة ثابتة — لا تُولَّد صفحات Sanity الديناميكية (متحدث/جلسة) فيها.
...
| السؤال | الجواب |
|---|---|
| هل الموقع مفهرس بالكامل؟ | نعم — `robots.txt` يسمح بكل شيء، و8 عناوين URL في `sitemap.xml` (4 صفحات × لغتين) مع `hreflang` صحيح |
