...
interface ActivationsHeroSectionProps {
  badgeLabel: string;
  mainTitle: string;
...
export default function ActivationsHeroSection({
  badgeLabel,
  mainTitle,
  highlightTitle,
  description,
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1000px] mx-auto px-5 sm:px-6 md:px-10 relative z-10 w-full flex flex-col items-center text-center">
        ...
        {/* العنوان الرئيسي بنفس ألوان وتدرج فريق العمل */}
        <div className="space-y-4 hero-fade-up w-full " style={{ animationDelay: '0.2s' }}>
          <h1 className={` text-[2.75rem] sm:text-5xl md:text-7xl lg:text-[6rem] font-black leading-[1.1] md:leading-[1.05] ${isArabic ? "font-arabic tracking-normal" : "font-sans tracking-[-0.04em]"}`}>
        ...
        <div className="mt-12 md:mt-16 flex flex-col items-center gap-8 hero-fade-up" style={{ animationDelay: '0.5s' }}>
          <div className="flex flex-col items-center gap-3 text-zinc-500 hover:text-white transition-colors cursor-pointer">
            <span className={`text-[10px] sm:text-xs font-bold uppercase tracking-[0.3em] ${isArabic ? 'font-arabic' : 'font-sans'}`}>{discoverLabel}</span>
      ...
      {/* أنيميشن CSS الضروري لإظهار العناصر */}
      <style>{`
        ...
        .hero-scale-x {
          transform: scaleX(0);
          animation: hero-scale-x 0.9s cubic-bezier(0.23, 1, 0.32, 1) forwards;
