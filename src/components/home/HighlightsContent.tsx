...
/* ═══════════════════════════════════════════
   مكونات فرعية
   ═══════════════════════════════════════════ */
const ArrowLink = memo(function ArrowLink({ text }: { text: string }) {
  return (
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-stretch">
        <div className="flex-1 p-8 md:p-10 flex flex-col justify-center gap-5">
          <div className="flex items-center gap-3">
            ...
            <SectionBadge>{venueBadgeText}</SectionBadge>
          </div>
...
const ActivationsCard = memo(function ActivationsCard({
  activationsTitle,
  activationsTeaser,
    ...
    >
      ...
      <div className="relative z-10 flex flex-col gap-5">
        <div className="flex items-center gap-3">
          ...
          <SectionBadge>{activationsBadgeText}</SectionBadge>
        </div>
  ...
  return (
    <motion.div variants={cardVariants}
      className="group relative rounded-[24px] bg-card border border-border overflow-hidden cursor-pointer h-full
        hover:border-tedx-red/30 hover:shadow-[0_24px_48px_-12px_rgba(0,0,0,0.1)]
        hover:-translate-y-1 transition-all duration-500 p-8 flex flex-col justify-between"
    >
      <div className="relative z-10 flex flex-col gap-5">
        <div className="flex items-center gap-3">
        ...
        <p className="text-sm text-muted-foreground leading-[1.8]">{dateDescription}</p>
      </div>
      <div className="mt-6 h-[3px] w-full bg-border rounded-full overflow-hidden">
        <div className="h-full w-0 group-hover:w-full bg-gradient-to-r from-tedx-red to-red-400 rounded-full transition-all duration-700 ease-out" />
      </div>
...
const SpeakerCTACard = memo(function SpeakerCTACard({
  speakerBadgeText,
  speakerTitle,
  speakerDescription,
  speakerCta,
        ...
        <div className="container-padding max-w-7xl mx-auto">
          ...
          >
            <VenueCard
              ...
              venueTeaser={venueTeaser}
              venueBadgeText={venueBadgeText}
              venueLinkText={venueLinkText}
            />
            <ActivationsCard
              activationsTitle={activationsTitle}
              activationsTeaser={activationsTeaser}
              activationsBadgeText={activationsBadgeText}
              activationsLinkText={activationsLinkText}
            />
            <StatsCard
              statsBadgeText={statsBadgeText}
