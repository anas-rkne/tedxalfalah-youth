  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 h-full w-full bg-gradient-to-br from-red-100/20 via-orange-100/15 to-yellow-100/10" aria-hidden="true">
      ...
      <motion.div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 20% 50%, rgba(220,38,38,0.1) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(251,146,60,0.08) 0%, transparent 40%)",
        }}
        animate={{
      ...
      <motion.div
        ...
        style={{
          ...
          y: smoothY,
          background:
            "radial-gradient(rgba(220,38,38,0.15) 0%, rgba(220,38,38,0.05) 60%, transparent 80%)",
        }}
      />

      {/* النجوم المتلألئة (جديدة) */}
      {memoizedStars.map((s) => (
        <motion.div
          key={s.id}
      ...
      {/* الرسومات المتحركة (مع تباين الحجم والعمق) */}
      {memoizedDoodles.map((d, index) => (
        <motion.div
          key={index}
          ...
          style={{
            top: `${d.top}%`,
            left: `${d.left}%`,
            filter: "drop-shadow(0 0 2px rgba(220,38,38,0.1))", // تأثير توهج خفيف
          }}
          animate={{
          ...
          transition={{
            duration: d.size / 4 + 5, // الأيقونات الكبيرة تتحرك أبطأ، الصغيرة أسرع
            repeat: Infinity,
            ease: "easeInOut",
          }}
        >
