...
import ActivationsHeroSection from "@/components/activations/ActivationsHeroSection";
...
export default async function ActivationsPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-[#e62b1e] selection:text-white pb-32 overflow-hidden font-sans relative">
      ...
      {/* ═══════════════════════════════════════════════════════════════
          HERO SECTION — مطابق لصفحة الفريق
          ═══════════════════════════════════════════════════════════════ */}
      <ActivationsHeroSection
      ...
      <section className="section-padding bg-background">
        <div className={`container-padding max-w-5xl mx-auto text-center ${isArabic ? 'font-arabic' : 'font-sans'}`}>
          ...
          <p className={`text-center max-w-2xl mx-auto mt-6 text-lg leading-relaxed text-muted-foreground ${isArabic ? 'font-arabic md:text-right' : ''}`}>
            {t("subtitle")}
          </p>
        </div>
      </section>
